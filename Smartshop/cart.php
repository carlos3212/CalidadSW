   ...
   <div class="container scroll info">
     <table class="highlight">
        ...
        <tbody>
          ...
          product.id , product.price as 'price',
          category.name as 'category', command.id as 'com' ,command.userid, command.statut,
          command.quantity as 'quantity'
FROM category, product, command
WHERE command.id_produit = product.id AND 
product.id_category = category.id AND command.userid = $_SESSION[userid] AND command.statut = 'ordered'";
          $result1 = $connection->query($queryproduct);
          if ($result1->num_rows > 0) {
          // output data of each row
          while($rowproduct = $result1->fetch_assoc()) {
            $id_productdb = $rowproduct['com'];
   ...
   <?php
    ...
    $tiempo_carga = require_once('includes/tiempo1.php'); 
    require_once('includes/tiempo1.php');
    add_cargapagina($conn, $tiempo_carga, $page_id) ?>
